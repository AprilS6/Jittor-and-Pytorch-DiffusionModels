...
class TimeEmbedding(nn.Module):
    ...
    def execute(self, t: jt.Var) -> jt.Var:
        ...
        emb = self.mlp(emb).clamp(emb, -1.0 + 1e-6, 1.0 - 1e-6) # MLP进一步提取可学习特征，以及调整维度
        return emb
...
class ResBlock(nn.Module):
    ...
    def forward(self, x: jt.Var, emb_t: jt.Var):
...
class AtnBlock(nn.Module):
    ...
    def execute(self, x: jt.Var, emb_t: jt.Var | None = None):
        ...
        qkv = self.proj(self.norm(x)).view(batch_size, -1, self.n_heads, 3 * self.k_dim) # 维度对齐，方便切分qkv -- (b, h*w, n_heads, 3*k_dim)
        q, k, v = jt.chunk(qkv, 3, dim=-1) # 切分，得到每个元素的qkv -- (b, h*w, n_heads, k_dim)
        # 注意力权重计算
        q.permute(0, 2, 1, 3) # (b, n_heads, i, k_dim)
        k.permute(0, 2, 3, 1) # (b, n_heads, k_dim, j)
        v.permute(0, 2, 1, 3) # (b, n_heads, j, k_dim)
